...
def imwrite(file_path,img,use_thread=False):
    ...
    if use_thread:
        t = Thread(target=subfun,args=(file_path, img,),daemon=True)
        t.start()
    else:
...
def splice(imgs,splice_shape,dst=None):
    ...
    h,w,ch = imgs[0].shape
    if dst is None:
        dst = np.zeros((h*splice_shape[0],w*splice_shape[1],ch),np.uint8)
    cnt = 0
    for i in range(splice_shape[0]):
        for j in range(splice_shape[1]):
            if cnt < len(imgs):
                dst[h*i:h*(i+1),w*j:w*(j+1)] = imgs[cnt]
                cnt += 1
